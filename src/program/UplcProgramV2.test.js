...
describe(UplcProgramV2.name, () => {
    it("evaluates always_fails as error", () => {
        const { result } = UplcProgramV2.fromCbor(
    ...
    it("evaluates always_succeeds as non-error", () => {
        const program = UplcProgramV2.fromCbor("4e4d01000033222220051200120011")
    ...
    it(`evaluates (program 1.0.0 (con bool false)) as UplcBool(false)`, () => {
        const program = UplcProgramV2.fromString(
...
describe(`${UplcProgramV2.name} conformance`, () => {
    ...
    runtimeErrorTests.forEach((s) => {
        it(`fails to run ${s}`, () => {
            const program = UplcProgramV2.fromString(s)
